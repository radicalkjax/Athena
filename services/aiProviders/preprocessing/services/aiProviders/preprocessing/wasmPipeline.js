    ...
    async initialize() {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn('Failed to initialize network bridge, URL sanitization will be limited', { error: bridgeError });
            ...
            logger_1.logger.error('Failed to initialize WASM preprocessing', { error: error instanceof Error ? error.message : String(error) });
            throw error;
        }
    }
    /**
    ...
    async processBinary(content) {
        ...
        try {
            ...
            if (fileAnalysis.suspicious_indicators && fileAnalysis.suspicious_indicators.length > 0) {
                result.safe = false;
                result.threats = fileAnalysis.suspicious_indicators.map((pattern) => ({
            ...
            if (fileAnalysis.strings) {
                const safeStrings = fileAnalysis.strings
                    .filter((str) => !str.suspicious && !this.containsMaliciousPattern(str.value))
                    .map((str) => str.value)
                    .join('\n');
