...
let CachedAIOrchestrator = (() => {
    ...
    return _a = class CachedAIOrchestrator {
             ...
             */
            async getCachedAnalysis(request, strategy) {
                ...
                // Compute and cache
                const response = await this.orchestrator.analyzeWithStrategy(request, strategy);
                // Determine TTL based on analysis type
                const ttl = this.determineTTL(request.analysisType);
                // Cache the response
                await this.cache.set(cacheKey, response, ttl, ['ai-analysis', request.analysisType, request.provider || 'auto']);
                return response;
            }
            /**
            ...
            async batchAnalyze(requests) {
                ...
                const cachedResponses = await this.cache.mget(cacheKeys);
                ...
                if (uncachedRequests.length > 0) {
                    const newResponses = await Promise.all(uncachedRequests.map(req => this.orchestrator.analyze(req)));
                    ...
                    for (let i = 0; i < newResponses.length; i++) {
                        ...
                        // Cache the response
                        await this.cache.set(this.generateCacheKey(request), response, this.determineTTL(request.analysisType), ['ai-analysis', request.analysisType]);
             ...
             */
            async invalidateCache(analysisType, provider) {
                if (analysisType) {
             ...
             */
            async warmupCache(commonRequests) {
