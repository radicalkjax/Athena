...
function benchmarkData(patternMatcher, testName, data, results) {
    return __awaiter(this, void 0, void 0, function* () {
...
function generateMaliciousScript(size) {
    const patterns = [
        'eval(atob("',
...
function generateMixedContent(size) {
    let content = '';
...
function generateObfuscatedContent(size) {
    let content = '';
...
function generateCleanData(size) {
...
function exportToCSV(results) {
    const csv = [
        ...
        ...results.map(r => `"${r.testName}",${r.fileSize},${r.fileSizeMB.toFixed(2)},${r.scanTimeMs.toFixed(2)},${r.throughputMBps.toFixed(2)},${r.matchCount},${r.threatScore.toFixed(2)},${r.rulesEvaluated}`)
    ].join('\n');
